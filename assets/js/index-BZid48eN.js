import{j as e,m as x}from"./motion-vendor-ubJsSJQ6.js";import{r as s}from"./react-vendor-DTA-bILe.js";import{J as T,K as L,M as S,N as k,O as F,P as I,Q as w,R as z,S as B,u as _,L as C,a as E}from"./index-DPDc_nCk.js";function P(r={}){const{vertexShader:n,fragmentShader:o,uniforms:l=[],attributes:m=[],onFrame:t,autoResize:d=!0}=r,i=s.useRef(null),a=s.useRef(null),u=s.useRef(null),v=s.useRef({}),b=s.useRef({}),p=s.useRef(null),[h,g]=s.useState({gl:null,canvas:null,isInitialized:!1,error:null}),j=s.useCallback(()=>{if(!i.current)return;if(!T()){g(f=>({...f,error:"WebGL is not supported in this browser"}));return}const c=L(i.current);if(!c){g(f=>({...f,error:"Failed to create WebGL context"}));return}a.current=c,d&&S(i.current,c),g({gl:c,canvas:i.current,isInitialized:!0,error:null})},[d]),R=s.useCallback(()=>{if(!a.current||!n||!o)return;const c=k(a.current,n,o);if(!c){g(f=>({...f,error:"Failed to compile shader program"}));return}u.current=c,v.current=F(a.current,c,l),b.current=I(a.current,c,m)},[n,o,l,m]),y=s.useCallback(()=>{if(!a.current||!u.current||!t)return;const c=a.current,f=u.current;c.useProgram(f),t(c,f,v.current),p.current=requestAnimationFrame(y)},[t]);return s.useEffect(()=>(j(),()=>{p.current&&cancelAnimationFrame(p.current),a.current&&u.current&&w(a.current,u.current)}),[j]),s.useEffect(()=>{h.isInitialized&&n&&o&&R()},[h.isInitialized,n,o,R]),s.useEffect(()=>(h.isInitialized&&u.current&&t&&y(),()=>{p.current&&(cancelAnimationFrame(p.current),p.current=null)}),[h.isInitialized,t,y]),s.useEffect(()=>{if(!d||!i.current||!a.current)return;const c=()=>{i.current&&a.current&&S(i.current,a.current)};return window.addEventListener("resize",c),()=>window.removeEventListener("resize",c)},[d]),{canvasRef:i,gl:h.gl,program:u.current,uniforms:v.current,attributes:b.current,isInitialized:h.isInitialized,error:h.error}}const A=s.memo(({vertexShader:r,fragmentShader:n,uniforms:o=[],attributes:l=[],onFrame:m,className:t,style:d,width:i,height:a})=>{const{canvasRef:u,error:v}=P({vertexShader:r,fragmentShader:n,uniforms:o,attributes:l,onFrame:m,autoResize:!i&&!a});return v?e.jsx("div",{className:t,style:d,children:e.jsxs("p",{children:["WebGL Error: ",v]})}):e.jsx("canvas",{ref:u,className:t,style:{display:"block",width:i?`${i}px`:"100%",height:a?`${a}px`:"100%",...d},width:i,height:a})});A.displayName="ShaderCanvas";const G=`// Simple gradient fragment shader
